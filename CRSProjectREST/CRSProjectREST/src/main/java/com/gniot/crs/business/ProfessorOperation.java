...
public class ProfessorOperation implements ProfessorInterface {
	...
	public void addGrade(int studentId, int courseId, String grade, int professorId,int gradeId) {
	    try {
	        System.out.println("Invoking professorDAO.addGrade()");
	        professorDAO.addGrade(studentId, courseId, grade, professorId,gradeId);
	...
	public List<Student> viewEnrolledStudents(int professorId) {
	    List<Student> students = new ArrayList<>(); // Initialize students to avoid returning null

	    try {
	        if (professorId == -1) {
	            System.out.println(RED + "Error: Professor not found." + RESET);
	            return students; // Return empty list if professor is not found
	        }

	        System.out.println("Fetching enrolled students for professor " + professorId);
	        students = professorDAO.getEnrolledStudents(professorId);
	        if (students.isEmpty()) {
	...
	public List<Course> getProfessorCourses(int professorId) {
		return professorDAO.getProfessorCourses(professorId);
	}
